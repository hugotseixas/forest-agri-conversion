...
The conversions were calculated pixel by pixel, where the LULC time series were extracted and analysed for each conversion cycle that was present for that pixel.
The calculation of the conversion length was extremely simple, it is the year of the establishment of agriculture subtracted by the last year classified as a forest, however, we add one year to the latter, so that it represents the year of deforestation (since it is more meaningful for analysis).

```{r routine-plot, out.width = "17cm", fig.pos = "ht", echo = FALSE, fig.cap = "Representation of the process to calculate the conversion length from forests to agriculture. All values presented are hiphothetical, and serves only as an ilustration of the method."}

knitr::include_graphics("figs/change_lenght_routine_scheme.png")
```

The process of calculation of the conversion length (Figure \@ref(fig:routine-plot)), can be breakdown to seven steps of processing:

1.  Load raster and extract valid values into a table;

2.  Calculate the year of first occurrence of any agriculture class (annual and permanent crops) for each pixel;
...
In addition to the conversion length values and the years of the conversion, they are also qualified by the type of agriculture that were established after deforestation, if the conversion occurred from primary or secondary forests, and the number of occurences of conversions at the pixel.
...
```{r tables-plot, out.width = "17cm", fig.pos = "ht", echo = FALSE, fig.cap = " Representation of the tables generated by the conversion length calculation. All values presented are hiphothetical, and serves only as an ilustration of the method."}

knitr::include_graphics("figs/tables_scheme.png")
```

The first type of table (Figure \@ref(fig:tables-plot).a) contain the spatial information (longitude, latitude and area) of each pixel, its unique identificators and the code of the municipality which contains the pixel.
...
This table is named as "mask_cells".
...
The "agri_code" column holds information about the type of agriculture the was established after deforestation.
...
Since it is possible that one pixels contains more than one conversion from forest to agriculture, we stored the number of the conversion in the column "c_cycle" (1 is the first conversion cycle).
However, conversion from forest to other LULC is also possible, so the column "f_cycle" holds information about the number of the conversion from forest to any LULC.
If the forest have suffered any conversion, it starts being considered as a secondary forest, as indicated in the column "forest_type".
...
This structure allows for easily access parts of the data set, and also filter it without the necessity to read the tables.
In the case of this data set, it is partitioned by the "tile_id" and "agri_cycle" variables.
The data set also have a metadata file with the spatial characteristics of each file, it stores the identification of the tile, the maximum and minimum latitude and longitude, the coordinate reference system, and the number of columns, rows, and cells of each tile.

```{r dataset-plot, out.width = "17cm", fig.pos = "ht", echo = FALSE, fig.cap = " Representation of the data set, structured in folders and sub folders."}
...
The period between 2001 and 2005 showed a peak of deforestation (Figure \@ref(fig:transbar-plot).a), and of croplands expansion (Figure \@ref(fig:transbar-plot).b).
...
```{r errorscatter-plot, out.width = "17cm", fig.pos = "ht", echo = FALSE, fig.cap = "Scatter plot between paired values of estimated and observed Deforestation and Agriculture Establishment years, and their respective Transition Length. The gray line represents values which there would be no errors of the estimated values in comparison with the observed values. Translucid white areas represent quantile ranges (5\\%, 25\\%, 75\\% and 95\\%) of the errors. The area between 5\\% and 95\\% quantiles includes 90\\% of points. The area between 25\\% and 75\\% quantiles includes 50\\% of points."}
