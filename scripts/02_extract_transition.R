...
for (tile in pull(.data = file_list, var = tile_id)) {

  ### Load one file as a raster stack ----
  mb_raster <- rast(x = file_list$mb_lulc[tile])
  ...
  tiles_metadata %<>%
    add_row(
      ...
      x_min = xmin(mb_raster),
      x_max = xmax(mb_raster),
      y_min = ymin(mb_raster),
      y_max = ymax(mb_raster),
      n_col = ncol(mb_raster),
      n_row = nrow(mb_raster),
      n_cell = ncell(mb_raster),
      crs = as.character(crs(mb_raster, proj4 = TRUE))
    )
  ...
  lulc_table <-
    ...
    select(-c(coords, area, code_muni))
  ...
  if (nrow(lulc_table) == 0) {
    ...
    ## Remove tile_id column from mask subset ----
    mask_subset %<>%
      select(-tile_id)
